...
public class CheckingExistenceofEdgeLengthLimitedPaths {
    public static void main(String[] args) {
        ...
        /*boolean[] result = new CheckingExistenceofEdgeLengthLimitedPaths().distanceLimitedPathsExist(
                5, new int[][]{{0,1,10},{1,2,5},{2,3,9},{3,4,13}},
    ...
    public boolean[] distanceLimitedPathsExist(int n, int[][] edgeList, int[][] queries) {
        ...
        for(int[] i:edgeList){
            HashMap<Integer,Integer> m1 = map.getOrDefault(i[0],new HashMap<Integer,Integer>());
            m1.put(i[1],Math.min(m1.getOrDefault(i[1],Integer.MAX_VALUE),i[2]));
            HashMap<Integer,Integer> m2 = map.getOrDefault(i[1],new HashMap<Integer,Integer>());
            m2.put(i[0],Math.min(m2.getOrDefault(i[0],Integer.MAX_VALUE),i[2]));
            map.put(i[0],m1);
            map.put(i[1],m2);
        }
        ...
        for(int i=0;i<queries.length;i++){
            result[i]=helper(queries[i][0],queries[i][0],queries[i][1],new boolean[n],0,map,dp)<queries[i][2];
        }
        return result;
    }
    private int helper(int root,int n1,int n2,boolean[] visited,int curMax,HashMap<Integer,HashMap<Integer,Integer>> map,int[][] dp){
        //if(n1==n2)
            return 0;
        /*if(visited[n1])
            return false;*/
